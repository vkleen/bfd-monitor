...
async def monitor_peer(peer_resp):
    async with aio.insecure_channel('unix:///run/bfdd/bfdd.sock') as channel:
        ...
        async for monitor_resp in stub.MonitorPeer(bfdd_pb2.MonitorPeerRequest(uuid = peer_resp.uuid)):
            ...
            if (monitor_resp.local.state == bfdd_pb2.SessionState.DOWN or
                (monitor_resp.remote.state == bfdd_pb2.SessionState.UP and monitor_resp.local.state == bfdd_pb2.SessionState.UP)):
                await peer.softreset(peer_resp.peer.name)
